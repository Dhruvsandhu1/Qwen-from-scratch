...
from transformers.utils.generic import split_attention_implementation
from transformers.utils.import_utils import PACKAGE_DISTRIBUTION_MAPPING, is_tracing
...
def flash_attn_supports_top_left_mask():
    ...
    from transformers.integrations.npu_flash_attention import is_npu_fa2_top_left_aligned_causal_mask
...
def is_flash_attn_available():
    return is_flash_attn_4_available() or is_flash_attn_3_available() or is_flash_attn_2_available() or is_torch_npu_available() or is_torch_xpu_available()
...
FLASH_ATTN_KERNEL_FALLBACK = {
    ...
    "flash_attention_3": ("kernels-community/aiter-flash-attn" if is_rocm_platform() else "kernels-community/vllm-flash-attn3"),
...
FLASH_ATTENTION_COMPATIBILITY_MATRIX = {
    2: {
        ...
        "pkg_availability_check": lambda *args, **kwargs: (importlib.util.find_spec("flash_attn") is not None and "flash-attn" in [pkg.replace("_", "-") for pkg in PACKAGE_DISTRIBUTION_MAPPING.get("flash_attn", [])]),
        "supported_devices": (
    ...
    3: {
        ...
        "pkg_availability_check": lambda *args, **kwargs: (importlib.util.find_spec("flash_attn_interface") is not None and "flash-attn-3" in [pkg.replace("_", "-") for pkg in PACKAGE_DISTRIBUTION_MAPPING.get("flash_attn_interface", [])]),
    ...
    4: {
        ...
        "pkg_availability_check": lambda *args, **kwargs: (importlib.util.find_spec("flash_attn") is not None and "flash-attn-4" in [pkg.replace("_", "-") for pkg in PACKAGE_DISTRIBUTION_MAPPING.get("flash_attn", [])]),
        "supported_devices": ((is_torch_cuda_available, "cuda"),),
...
def _lazy_imports(implementation: str | None, attention_wrapper: Callable | None = None, allow_all_kernels: bool = False):
    """
    ...
    if (implementation == "flash_attention_2" and is_fa2) or (implementation is None and is_fa2 and not is_fa3 and not is_fa4):
        ...
        from transformers.integrations.npu_flash_attention import npu_flash_attn_with_kvcache as flash_attn_with_kvcache
    else:
        ...
        # Kernels fallback
            ...
            kernel = load_and_register_attn_kernel(kernel_implementation, attention_wrapper, allow_all_kernels=allow_all_kernels)
            ...
            if flash_attn_varlen_func is None:
                raise ValueError(f"Could not find the currently requested flash attention implementation at `{implementation}`." "Make sure that you request a valid kernel from the hub, e.g. `kernels-community/flash-attn2`.")
            if flash_attn_func is None:
                logger.warning(f"The loaded flash attention implementation at `{implementation}` only supports varlen, i.e. " "it can only be used with continuous batching and does not support the full functionality for " "the base transformers generation methods.")
            if flash_attn_with_kvcache is None:
                logger.warning(f"The loaded flash attention implementation at `{implementation}` does not support block tables, so" " the full performances of continuous batching will not be achieved, only the varlen path will be " "used.")
...
def lazy_import_flash_attention(implementation: str | None, attention_wrapper: Callable | None = None, allow_all_kernels: bool = False):
    """
    ...
    if implementation is not None and _loaded_implementation != implementation:
        ...
        _flash_fn, _flash_varlen_fn, _flash_with_kvcache_fn, _pad_fn, _unpad_fn = _lazy_imports(implementation, attention_wrapper, allow_all_kernels=allow_all_kernels)
...
def lazy_import_paged_flash_attention(implementation: str | None, allow_all_kernels: bool = False):
    ...
    (_, flash_attn_varlen_func, flash_attn_with_kvcache_fn, _, _), _ = lazy_import_flash_attention(implementation, attention_wrapper=paged_attention_forward, allow_all_kernels=allow_all_kernels)
...
def _process_flash_attention_kwargs(
    ...
    **kwargs,
    ...
    if supports_mapping["deterministic"]:
        flash_kwargs["deterministic"] = deterministic if deterministic is not None else os.getenv("FLASH_ATTENTION_DETERMINISTIC", "0") == "1"
...
def _flash_attention_forward(
    ...
    **kwargs,
    ...
    """
    (flash_fn, flash_varlen_fn, _, pad_fn, unpad_fn), process_flash_kwargs_fn = lazy_import_flash_attention(attn_implementation)

    # PEFT possibly silently casts tensors to fp32, this potentially reconverts to correct dtype or is a no op
    query_states, key_states, value_states = fa_peft_integration_check(query_states, key_states, value_states, target_dtype)
    ...
    is_fa_with_varlen_kwargs = all(kwarg is not None for kwarg in (cu_seq_lens_q, cu_seq_lens_k, max_length_q, max_length_k))

    # Contains at least one padding token in the sequence
    if attention_mask is not None:
        q, k, v, indices_q, (cu_seq_lens_q, cu_seq_lens_k), (max_length_q, max_length_k) = _upad_input(query_states, key_states, value_states, attention_mask, query_length, unpad_fn)
    ...
    # Padding free, i.e. sequences flattened into one total sequence
        ...
        if cu_seq_lens_q is None or cu_seq_lens_k is None:
            q, k, v, (cu_seq_lens_q, cu_seq_lens_k), (max_length_q, max_length_k) = _prepare_from_posids(query_states, key_states, value_states, position_ids)
        else:
